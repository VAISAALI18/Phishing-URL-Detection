...
class FeatureExtraction:
    ...
    def RequestURL(self):
        try:
            ...
            for tag in self.soup.find_all(['img', 'audio', 'embed', 'iframe'], src=True):
                ...
                if self.url in tag['src'] or self.domain in tag['src'] or len(dots) == 1:
                    success += 1
                i += 1
    ...
    def AnchorURL(self):
        try:
            ...
            for a in self.soup.find_all('a', href=True):
                href = a['href']
                if ("#" in href or "javascript" in href.lower() or "mailto" in href.lower() or
                        (self.url not in href and self.domain not in href)):
                    unsafe += 1
                i += 1
    ...
    def InfoEmail(self):
        try:
            if re.findall(r"mailto:", self.soup.prettify()):
                return -1
            else:
                return 1
        except:
    ...
    def PageRank(self):
        try:
            ...
            rank_match = re.findall(r"Global Rank: ([0-9]+)", prank_checker_response.text)
            if rank_match:
                global_rank = int(rank_match[0])
